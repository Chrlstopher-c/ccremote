...
describe('répertoire de compte — H-44 rendue effective (multi-machines, 01/08)', () => {
  ...
  const lecteurDepense = (): null => null;
  ...
  test('aucun compte local déclaré ⇒ comportement d’origine intact (mono-machine)', () => {
    const spec = construireWorkerSpec(
      { ...base, configDir: '/home/trinity/.claude-comptes/compte-a' },
      port,
      lecteurDepense,
    );
...
// ------------------------------------------- serveur MCP de consultation de la dépense

describe('☠ le serveur de consultation de la dépense est bien assemblé (mandat 18/08)', () => {
  const base = {
    sessionId: 's-1',
    cwd: '/mnt/projects/stockiop',
    mandate: 'm',
  ...
  test('exposé sous le nom `ccremote-depense`, EN-PROCESS, en plus des MCP du poste', () => {
    const spec = construireWorkerSpec({ ...base, maxBudgetUsd: 12 }, port, () => null);
    const serveur = spec.mcpServers['ccremote-depense'];
    expect(serveur).toBeDefined();
    // `type: 'sdk'` — assemblé par `createSdkMcpServer`, jamais un sous-processus
    // ni un réseau : c'est ce qui garantit l'absence de dépendance réseau fragile.
    expect((serveur as { type: string }).type).toBe('sdk');
  });
});
